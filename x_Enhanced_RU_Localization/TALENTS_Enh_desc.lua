...
--[+ +BLITZ+ +]--
	...
	local ED_PSY_Blitz_2_rgb = iu_actit(table.concat({
		...
		-- "- Невозможно ошеломить монстров и врагов с активным пустотным щитом.",
		-- "- Средние модификаторы урона по всем видам брони, низкие модификаторы урона по панцирной броне.",
		dmg_is_incr_by,
	...
	local ED_PSY_Blitz_2_2_rgb = iu_actit(table.concat({
		...
		"- Не суммируется с тем же ослаблением, наложенным другим псайкером.",
	...
	local ED_PSY_Blitz_2_3_rgb = iu_actit(table.concat({
		...
		"-- Обратите внимание, что время до того, как может произойти первое срабатывание урона, зависит от ударной массы противника (как и в случае с самим «Сокрушением), поэтому чем больше ударная масса противника, тем больше времени потребуется эффекту электрошока этого таланта, чтобы нанести урон. В результате против монстров (ударная масса 20), только 1 срабатывание урона может быть выполнено до того, как закончится окно урона в 2 секунды.",
		"-- Если выбрано «Ослабление», эффект электрошока получает более выгодную стоимость по ударной массе, что фактически удваивает количество его срабатываний против большинства врагов. С «Ослаблением» он также выигрывает от 10%-ного увеличения получаемого урона. По-видимому, каждый тик урона добавляет один стек дебаффа, увеличивая урон, который получает ослабленный враг, соответственно, за тик. Все атакующие могут извлечь из этого выгоду, только пока эффект электрошока активно наносит урон и применяет дебафф (как и в случае с самим «Smite). With Enfeeble, it also benefits from the 10% increased Damage taken debuff. Apparently, each Damage tick adds one debuff Stack increasing the Damage that the debuffed enemy takes accordingly per tick. All attackers can benefit from this only while the Electrocution effect is actively Damaging and applying the debuff (just like with Smite itself).",
	}, "\n"), enhdesc_col)
...
--[+ +ABILITIES+ +]--
	...
	local ED_PSY_Ability_1_2_rgb = iu_actit(table.concat({
		...
		"--- Melee, if procced with weapon before Warp Rupture's activation: \"Executor\", \"High Voltage\" (vs Electrocuted), \"Череподробитель\" (vs Staggered), \"Slaughterer\", \"Superiority\", and \"Unstable Power\".",
	...
	local ED_PSY_Ability_1_3_rgb = iu_actit(table.concat({
		...
		"- Soulblaze damage is increased by пробивания and хрупкости, by Perks of currently equipped Weapons, and by Buffs from Talents: \"Disrupt Destiny\", \"Empyrean Empowerment\", \"Malefic Momentum\", \"Kinetic Presence\", \"Perfect Timing\", and \"Warp Rider\".",
		"-- Blessings of the Weapons:",
		"--- Melee: \"Executor\", \"High Voltage\" (vs Electrocuted), \"Череподробитель\" (vs Staggered), \"Slaughterer\", \"Superiority\", \"Uncanny Strike\", and \"Unstable Power\".",
